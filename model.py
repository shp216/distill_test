...
class UNetWithFeatures(UNet2DModel):

    def __init__(self, *args, **kwargs):
        # 부모 클래스인 UNet2DModel의 초기화를 호출
        super(UNetWithFeatures, self).__init__(*args, **kwargs)
    
    def forward(self, x, t, **kwargs) :
        # 기본적인 UNet2DModel의 forward 동작을 그대로 사용
        return super().forward(x, t, **kwargs)
    
    def forward_features(self, x, t):
        # 중간 피처를 추출하는 추가 기능 (forward와는 별개로 동작)
        temb = self.time_embedding(t)
        
        # Downsampling
        h = self.conv_in(x)
        hs = [h]
        features = []  # 중간 피처를 저장할 리스트
        for down_block in self.down_blocks:
            h = down_block(h, temb)
            hs.append(h)
            features.append(h)  # 다운샘플링 중간 피처 저장
        
        # Middle blocks
        h = self.mid_block(h, temb)
        features.append(h)  # 미들 블록 피처 저장
        
        # Upsampling
        for up_block in self.up_blocks:
            h = torch.cat([h, hs.pop()], dim=1)  # 스킵 커넥션과 결합
            h = up_block(h, temb)
            features.append(h)  # 업샘플링 중간 피처 저장
        
        h = self.conv_norm_out(h)
        h = self.conv_act(h)
        h = self.conv_out(h)

        return h, features  # 최종 출력과 중간 피처 반환
