...
class distillation_cache_Trainer(nn.Module):
    ...
    def teacher_step(self, x_t, t):
        ...
        x_t_ = mean + torch.exp(0.5 * log_var) * noise
...
class GaussianDiffusion_joint_Sampler(nn.Module):
    def __init__(self, T_model, S_model, beta_1, beta_T, T, img_size=32,
                 mean_type='eps', var_type='fixedlarge'):
        assert mean_type in ['xprev' 'xstart', 'epsilon']
        assert var_type in ['fixedlarge', 'fixedsmall']
        super().__init__()
    ...
    def p_mean_variance(self, x_t, t):
        ...
        half = x_t.shape[0] // 2
        x_t_T = x_t[:half]  # First half for T_model
        t_T = t[:half]

        x_t_S = x_t[half:]  # Second half for S_model
        t_S = t[half:]

        # Mean parameterization
        if self.mean_type == 'xprev':
            x_prev_T = self.T_model(x_t_T, t_T)
            x_0_T = self.predict_xstart_from_xprev(x_t_T, t_T, xprev=x_prev_T)
            model_mean_T = x_prev_T
            x_prev_S = self.S_model(x_t_S, t_S)
            x_0_S = self.predict_xstart_from_xprev(x_t_S, t_S, xprev=x_prev_S)
            model_mean_S = x_prev_S
        elif self.mean_type == 'xstart':
            x_0_T = self.T_model(x_t_T, t_T)
            model_mean_T, _ = self.q_mean_variance(x_0_T, x_t_T, t_T)
            x_0_S = self.S_model(x_t_S, t_S)
            model_mean_S, _ = self.q_mean_variance(x_0_S, x_t_S, t_S)
        elif self.mean_type == 'epsilon':
            eps_T = self.T_model(x_t_T, t_T)
            x_0_T = self.predict_xstart_from_eps(x_t_T, t_T, eps=eps_T)
            model_mean_T, _ = self.q_mean_variance(x_0_T, x_t_T, t_T)
            eps_S = self.S_model(x_t_S, t_S)
            x_0_S = self.predict_xstart_from_eps(x_t_S, t_S, eps=eps_S)
            model_mean_S, _ = self.q_mean_variance(x_0_S, x_t_S, t_S)
        else:
            raise NotImplementedError(self.mean_type)
        
        x_0_T = torch.clip(x_0_T, -1., 1.)
        x_0_S = torch.clip(x_0_S, -1., 1.)

        model_mean = torch.cat([model_mean_T, model_mean_S], dim=0)
